...
function Marksmanship:st()
    ...
    if (MaxDps:FindSpell(classtable.BlackArrow) and CheckSpellCosts(classtable.BlackArrow, 'BlackArrow')) and cooldown[classtable.BlackArrow].ready then
    ...
    if (MaxDps:FindSpell(classtable.RapidFire) and CheckSpellCosts(classtable.RapidFire, 'RapidFire')) and (not talents[classtable.LunarStorm] or ( not cooldown[classtable.LunarStormIcd].ready==false or cooldown[classtable.LunarStormIcd].remains >5 )) and cooldown[classtable.RapidFire].ready then
    ...
    if (MaxDps:FindSpell(classtable.SteadyShot) and CheckSpellCosts(classtable.SteadyShot, 'SteadyShot')) and (talents[classtable.SteadyFocus] and not buff[classtable.SteadyFocusBuff].up and not buff[classtable.TrueshotBuff].up) and cooldown[classtable.SteadyShot].ready then
        return classtable.SteadyShot
    end
...
function Marksmanship:trickshots()
    ...
    if (MaxDps:FindSpell(classtable.Barrage) and CheckSpellCosts(classtable.Barrage, 'Barrage')) and (talents[classtable.RapidFireBarrage] and buff[classtable.TrickShotsBuff].remains >= timeShift) and cooldown[classtable.Barrage].ready then
        return classtable.Barrage
    end
    ...
    if (MaxDps:FindSpell(classtable.WailingArrow) and CheckSpellCosts(classtable.WailingArrow, 'WailingArrow')) and (not buff[classtable.PreciseShotsBuff].up) and cooldown[classtable.WailingArrow].ready then
        return classtable.WailingArrow
    end
    ...
    if (MaxDps:FindSpell(classtable.AimedShot) and CheckSpellCosts(classtable.AimedShot, 'AimedShot')) and (buff[classtable.TrickShotsBuff].remains >= timeShift and not buff[classtable.PreciseShotsBuff].up) and cooldown[classtable.AimedShot].ready then
        return classtable.AimedShot
    end
    if (MaxDps:FindSpell(classtable.MultiShot) and CheckSpellCosts(classtable.MultiShot, 'MultiShot')) and (not buff[classtable.TrickShotsBuff].up or buff[classtable.PreciseShotsBuff].up or Focus >MaxGetSpellCost(classtable.MultiShot, 'FOCUS') + MaxGetSpellCost(classtable.AimedShot, 'FOCUS')) and cooldown[classtable.MultiShot].ready then
        return classtable.MultiShot
    end
    if (MaxDps:FindSpell(classtable.SteadyShot) and CheckSpellCosts(classtable.SteadyShot, 'SteadyShot')) and cooldown[classtable.SteadyShot].ready then
        return classtable.SteadyShot
...
function Marksmanship:callaction()
    ...
    trueshot_ready = cooldown[classtable.Trueshot].ready and ( (targets <2) and ( not talents[classtable.Bullseye] or ttd >cooldown[classtable.Trueshot].duration + buff[classtable.TrueshotBuff].duration % 2 or buff[classtable.BullseyeBuff].count == buff[classtable.BullseyeBuff].maxStacks ) or (targets >1) and ( not (targets >1) and ( (targets>1 and MaxDps:MaxAddDuration() or 0) + math.huge <25 or math.huge >60 ) or (targets >1) and targets >10 ) or boss and ttd <25 )
