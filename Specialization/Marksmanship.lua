...
function Marksmanship:callaction()
    ...
    trueshot_ready = cooldown[classtable.Trueshot].ready and ((not (targets >1) or 1 == 1) and (not talents[classtable.Bullseye] or ttd >cooldown[classtable.Trueshot].duration+buff[classtable.TrueshotBuff].duration%2 or buff[classtable.BullseyeBuff].count == buff[classtable.BullseyeBuff].maxStacks) and (not MaxDps:HasOnUseEffect('13') or MaxDps:CheckTrinketCooldown('13').remains >30 or MaxDps:CheckTrinketReady('13')) and (not MaxDps:HasOnUseEffect('14') or MaxDps:CheckTrinketCooldown('14').remains >30 or MaxDps:CheckTrinketReady('14')) or (targets >1) and (not (targets >1) and ((targets>1 and MaxDps:MaxAddDuration() or 0) + math.huge<25 or math.huge >60) or (targets >1) and targets >10) or MaxDps:boss() and ttd <25)
